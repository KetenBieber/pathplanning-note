...
对于一个最优控制问题（最小化代价J，同时满足动力学）

状态必须按动力学演化（物理规则不可以违背）

状态λ必须按特定方程演化（从终点向后传播）

控制 u* 每时刻都让 Hamiltonian “最小”
...
$$
J_∑ = ∑_k J_k ,  J_k = (1/T) ∫_0^T j_k(t)^2 dt
$$
...
$$
p(T) = p₀ + v₀ T + (a₀/2) T² + (γ/6) T³ + (β/24) T⁴ + (α/120) T⁵ = p_f \\

v(T) = v₀ + a₀ T + (γ/2) T² + (β/6) T³ + (α/24) T⁴ = v_f \\
a(T) = a₀ + γ T + (β/2) T² + (α/6) T³ = a_f
$$
...
因此一般工程中，会这样设计：取两种启发式值的max

启发式函数作用：

h< = h_true

但是如果h离真值越远，那么其能起到的引导作用就越小

non-holonomic-without-obstacles 不能侧滑的不考虑障碍物的启发式
